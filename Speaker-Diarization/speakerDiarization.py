...
import librosa
import sys
import speech_recognition as speech_reg
...
#                     |-------------------|
#                               |-------------------|
def load_data(path,sr=16000, win_length=400,  hop_length=160, n_fft=512, embedding_per_second=0.5, overlap_rate=0.5):
...
def main(wav_path, embedding_per_second=1.0, overlap_rate=0.5):
    ...
    for spk in speakerSlice.keys():
        ...
        for item_dict in speakerSlice[spk]:
            ...
            filename = 'speaker' + str(spk) + '-' + str(item_dict['start'] / 1000) + '-' + str(
...
if __name__ == '__main__':
    start = time.time()
    slice=main(r'D:\University\OSU\t4mh\pilot4_mono.wav', embedding_per_second=1.2, overlap_rate=0.4)
    end = time.time()
